# instanovo_fm/eval/embedding_io.py
...
_LABEL_TO_CATEGORY: dict[str, Any] = {
    "b-ion": "fragment_base",
    "y-ion": "fragment_base",
    "a-ion": "fragment_base",
...
def _generate_theoretical_spectra_batch(
    ...
    theoretical_mz_list: list[Any] = []
    theoretical_annotations_list: list[Any] = []
    theoretical_match_mask_list: list[Any] = []
    theoretical_match_idx_list: list[Any] = []
    matched_annotation_list: list[Any] = []  # Annotations for matched experimental peaks
    feature_type_list: list[Any] = []  # For conditional annotation
    parent_annotation_list: list[Any] = []  # For conditional annotation
    ppm_error_list: list[Any] = []  # Per-peak ppm errors (NaN for unmatched)
    match_metrics_list: list[Any] = []  # Per-spectrum match summary metrics
    feature_type_detail_list: list[Any] = []  # Canonical peak labels (b-ion, y-loss, etc.)
    fragment_group_key_list: list[Any] = []  # Fragment group keys for completeness analysis
    spectrum_quality_list: list[Any] = []  # Per-spectrum backbone coverage & fragment group metrics
    ...
    for idx in tqdm(range(n_spectra), desc="Generating theoretical spectra", disable=not sys.stderr.isatty()):
        ...
        clean_sequence = DataProcessor.clean_peptide_for_pyopenms(seq_str, keep_modifications=keep_modifications)
        ...
        try:
            ...
            if keep_modifications:
                clean_sequence_unmod = DataProcessor.clean_peptide_for_pyopenms(seq_str, keep_modifications=False)
...
def generate(
        ...
        for batch_idx, batch in enumerate(iterator):
            ...
            if compute_confidence:
                try:
                    ...
                    if isinstance(preds, tuple):  # Classification model
                        ...
                        if n_groups is not None and last_group_size is not None:
                            ...
                            if store_per_peak_confidence:
                                ...
                                bin_size = (
                                    model.binning_strategy.bin_size
                                    if hasattr(model, "binning_strategy") and hasattr(model.binning_strategy, "bin_size")
                                    else None
                                )
                                ...
                                pred_mz_da = bin_groups_to_mz(
                                    pred_groups, pred_offsets, bin_size, group_size, min_mz, bin_edges=bin_edges
                                )  # (B, L) in Da
                    ...
                    spectrum_confidence = None  # type: ignore[assignment]
                    per_peak_confidence = None
                    per_peak_conf_group = None
                    per_peak_conf_offset = None
                    predicted_mz_normalized = None
    ...
    if "usi" in merged_metadata and "search_project" not in merged_metadata:
        try:
            ...
            unique_projects = {p for p in projects if p}
            logger.info(
                f"Extracted search_project: {n_valid}/{len(projects)} valid, "
    ...
    if "sequence" not in merged_metadata and "peptides" in merged_metadata:
        merged_metadata["sequence"] = merged_metadata["peptides"]
    ...
    if "sequence" in merged_metadata:
        try:
            ...
            is_valid = np.array([seq is not None and isinstance(seq, str) and len(seq.strip()) > 0 for seq in sequences], dtype=bool)
            ...
            if modification_types is not None and len(modification_types) > 0:
                ...
                if len(ranked_mods) > 0:
                    ...
                    modification_class = np.where(
                        single_mods == "Unmodified", "Unmodified", np.where(np.isin(single_mods, list(top_6_mods)), single_mods, "Other")
                    )
                    ...
                    for cls, cnt in zip(class_unique, class_counts, strict=False):
