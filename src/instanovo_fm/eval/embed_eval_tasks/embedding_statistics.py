...
class EmbeddingStatisticsTask(BaseTask):
    ...
    def run(self, emb: np.ndarray, meta: Dict[str, np.ndarray], faiss_index: Any) -> Dict[str, Any]:  # type: ignore[override]  # base class run() signature differs across tasks
        ...
        if self.create_dashboard:
            try:
                ...
                warnings.warn(f"Failed to create quality dashboard: {e}", stacklevel=2)

        return results

    def _compute_basic_stats(self, emb: np.ndarray, norms: np.ndarray, is_l2_normalized: bool, D: int) -> Dict[str, Any]:  # noqa: N803
    ...
    def _compute_pca_analysis(self, emb: np.ndarray, D: int) -> Dict[str, Any]:  # noqa: N803
        ...
        try:
            ...
            def _nth_component_for(threshold: Any, cum: Any) -> Any:
    ...
    def _create_quality_dashboard(self, results: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            D = results.get("embedding_dim", 768)  # noqa: N806
            ...
            if "similarity_distribution" in results and "error" not in results["similarity_distribution"]:
                ...
                ax1.text(0.5, 0.5, "N/A\n(Similarity distribution not computed)", ha="center", va="center", transform=ax1.transAxes, fontsize=12)
                ax1.set_title("Pairwise Similarity Distribution")
            ...
            if "pca_analysis" in results and "error" not in results["pca_analysis"]:
                ...
                if eigenvalues is not None:
                    ...
                    ax2.axhline(mean_eigenvalue, color="gray", linestyle=":", linewidth=1.5, alpha=0.7, label=f"Mean: {mean_eigenvalue:.2e}")
                ...
                ax2.text(0.5, 0.5, "N/A\n(PCA analysis not computed)", ha="center", va="center", transform=ax2.transAxes, fontsize=12)
                ax2.set_title("Eigenvalue Spectrum")
            ...
            if "pca_analysis" in results and "error" not in results["pca_analysis"]:
                ...
                if cumulative_variance_list is not None:
                    ...
                    ax3.plot(components, iso_cumvar, "gray", linestyle=":", linewidth=1.5, alpha=0.7, label=f"Isotropic (D={D})")
                ...
                ax3.text(0.5, 0.5, "N/A\n(PCA analysis not computed)", ha="center", va="center", transform=ax3.transAxes, fontsize=12)
                ax3.set_title("PCA Cumulative Variance")
            ...
            warnings.warn(f"Failed to create dashboard: {e}", stacklevel=2)
            if "fig" in locals():
                plt.close(fig)
            return None
