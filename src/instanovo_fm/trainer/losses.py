...
def compute_classifier_confidence(
    ...
    """Compute per-peak top-1 probability for the group and offset classification heads, plus their product as the joint confidence.

    Args: group_logits: Group logits of shape (B, L, n_groups) offset_logits: Offset logits of shape (B, L, group_size) n_groups: Total number of
    groups last_group_size: Number of valid bins in last group Returns: Dictionary with: - conf_group: Group top-1 probability (B, L) - conf_offset:
    Offset top-1 probability (B, L), computed after masking invalid offsets in the last (partial) group - conf_joint: conf_group * conf_offset (B,
    L)
    """
    B, L, _ = group_logits.shape  # noqa: N806
    ...
    offset_logits_masked_flat = mask_invalid_offsets(offset_logits_flat, group_pred_flat, n_groups, last_group_size)
...
def compute_classification_loss(
    ...
    target_groups, target_offsets = mz_to_bin_groups(target_mz_scaled, bin_size, max_mz, group_size, min_mz, bin_edges=bin_edges)
...
def compute_ptm_loss(logits: torch.Tensor, labels: torch.Tensor, gamma: float = 2.0, alpha: float = 0.25) -> torch.Tensor:
...
def compute_dmz_loss(logits: torch.Tensor, labels: torch.Tensor, gamma: float = 2.0, alpha: float = 0.25) -> torch.Tensor:
...
def compute_intensity_regression_loss(predictions: torch.Tensor, targets: torch.Tensor, delta: float = 0.1) -> torch.Tensor:
...
def compute_auxiliary_losses(
    aux_out: Dict[str, Any], batch: Dict[str, torch.Tensor], aux_config: Dict, model_config: Dict
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Compute the auxiliary task losses.

    Covers charge classification, retention-time regression, Δm/z classification,
    PTM classification and intensity regression.
    ...
    if aux_out and isinstance(aux_out, dict):
        for _key, value in aux_out.items():
    ...
    if lambda_dmz > 0.0 and "dmz" in aux_out and "dmz_labels" in batch:
        dmz_loss = compute_dmz_loss(
            aux_out["dmz"], batch["dmz_labels"], gamma=aux_config.get("dmz_focal_gamma", 2.0), alpha=aux_config.get("dmz_focal_alpha", 0.25)
        )
    else:
    ...
    if lambda_ptm > 0.0 and "ptm" in aux_out and "ptm_present" in batch:
        ptm_loss = compute_ptm_loss(
            aux_out["ptm"], batch["ptm_present"], gamma=aux_config.get("ptm_focal_gamma", 2.0), alpha=aux_config.get("ptm_focal_alpha", 0.25)
        )
    else:
    ...
    if lambda_intensity > 0.0 and "intensity" in aux_out and "intensity_labels" in batch:
        ...
        intensity_loss = compute_intensity_regression_loss(
            aux_out["intensity"], masked_intensity_labels, delta=intensity_head_cfg.get("huber_delta", 0.1)
        )
    else:
    ...
    total_aux_loss = (
        lambda_charge * charge_loss + lambda_rt * rt_loss + lambda_dmz * dmz_loss + lambda_ptm * ptm_loss + lambda_intensity * intensity_loss
    )
