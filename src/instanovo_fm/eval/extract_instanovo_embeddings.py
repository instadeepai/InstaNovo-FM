...
Usage:
    uv run python -m instanovo_fm.eval.extract_instanovo_embeddings \
        --parquet_path /path/to/data.parquet \
        --output_dir /path/to/output/instanovo \
        --checkpoint_path /path/to/instanovo_extended.ckpt \
        [--batch_size 64] \
        [--device cuda] \
        [--max_samples 10000] \
...
Checkpoint download:
    The instanovo_extended checkpoint can be downloaded from:
    https://github.com/instadeepai/InstaNovo/releases/download/1.0.0/instanovo_extended.ckpt

Encoder pooling:
    Uses the last token of the encoder output (memories[:, -1, :]) as the
    spectrum-level embedding, following the latent token design in InstaNovo.

m/z denormalisation:
    FoundationalDataProcessor normalises m/z to [0, 1] by dividing by max_mz=2500.
    This script multiplies spectra[:, :, 0] by 2500 before passing to the InstaNovo
...
def extract(
    ...
    Args:
        ...
        pooling: Pooling strategy — "last_token" (latent token, default) or
            "mean_peaks" (mean over non-padding peak tokens, excluding the
            prepended precursor and latent tokens).
        search_data_path: Path to search_data.csv for USI-based frag_type and
            search_instrument lookup.
    """
    if pooling not in ("last_token", "mean_peaks"):
        raise ValueError(f"pooling must be 'last_token' or 'mean_peaks', got {pooling!r}")
    ...
    if not ckpt.exists():
        if checkpoint_url:
            ...
            raise FileNotFoundError(
                ...
                "Pass --checkpoint_url to download automatically, or download manually:\n"
                "  https://github.com/instadeepai/InstaNovo/releases/download/1.0.0/instanovo_extended.ckpt"
            )
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="InstaNovo embeddings")):
            ...
            # Clamp charges to [1, max_charge] so the charge embedding lookup
            # never receives index 0-1=-1 (charge=0) or > max_charge-1.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract InstaNovo encoder embeddings into foundation-model HDF5 format"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Local path to instanovo_extended.ckpt"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pooling", default="last_token", choices=["last_token", "mean_peaks"],
        help="Pooling strategy: 'last_token' (latent token, default) or 'mean_peaks'"
    )
    parser.add_argument(
        "--search_data_path",
