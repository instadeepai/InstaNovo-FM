...
class TransformerTrainer(AccelerateDeNovoTrainer):
    ...
    def save_model(self, is_best_checkpoint: bool = False) -> None:
        ...
        self._log_checkpoint_artifact(model_path, artifact_path="checkpoints")
        ...
        if is_best_checkpoint and self.accelerator.is_main_process:
            ...
            self._log_checkpoint_artifact(best_model_path, artifact_path="checkpoints")

            # Register best model with MLFlow Model Registry if configured
            mlflow_register_model = self.config.get("mlflow_register_model", False)
            if mlflow_register_model and self.tracker is not None:
                # Use config value as model name, or default to run_name
                model_name = mlflow_register_model if isinstance(mlflow_register_model, str) else self.config.get("run_name", "instanovo")
                # Get input example for model signature inference
                input_example = self.get_model_input_example()
