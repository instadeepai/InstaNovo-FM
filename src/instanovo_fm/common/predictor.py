...
from instanovo.constants import ANNOTATED_COLUMN, ANNOTATION_ERROR, PREDICTION_COLUMNS
...
from instanovo_fm.utils.git_utils import get_user
...
class AccelerateDeNovoPredictor(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if self.tracker is not None:
            self.tracker.log_dataset(
                context="evaluation",
                name=str(self.config.get("run_name", "predict_evaluation")),
                source=str(self.config.get("data_path", "unknown")),
    ...
    def setup_tracking(self) -> None:
        """Setup MLFlow tracking for evaluation-mode prediction runs."""
        if not self.accelerator.is_main_process or self.denovo or not self.config.get("mlflow_enabled", True):
            self.tracker = None
            return
        ...
        self.tracker.log_hparams({k: v for k, v in self.config.items() if isinstance(v, (int, float, str, bool))}, {})
        self.tracker.log_param("started_by", get_user())
        self.tracker.log_param("predict.mode", "evaluation")
        self.tracker.log_param("predict.run_name", str(self.config.get("run_name", "")))
    ...
    def load_dataset(self) -> Dataset:
        ...
        dataset = dataset.filter(lambda row: (row["precursor_charge"] <= max_charge) and (row["precursor_charge"] > 0))
        ...
        if self._group_mapping is not None:
            ...
            groups = ["no_group"] * len(dataset)
            self.using_groups = False

        unique_groups = sorted(set(groups))
        self.group_to_idx: dict[str, int] = {g: i for i, g in enumerate(unique_groups)}
        self.group_idx_to_name: dict[int, str] = {i: g for g, i in self.group_to_idx.items()}
        group_codes = [self.group_to_idx[g] for g in groups]
        dataset = dataset.add_column("group", group_codes, feature=Value("int32"))
    ...
    def predict(self) -> pd.DataFrame:
        ...
        try:
            ...
            for i, batch in enumerate(self.test_dataloader):
                ...
                batch_predictions["group"] = [self.group_idx_to_name[x.item() if hasattr(x, "item") else int(x)] for x in batch["group"]]
    ...
    def calculate_metrics(
        ...
        if self.tracker is not None:
            self.tracker.log_scalar("predict/aa_er", aa_er)
            self.tracker.log_scalar("predict/aa_prec", aa_prec)
            self.tracker.log_scalar("predict/aa_recall", aa_recall)
            self.tracker.log_scalar("predict/pep_prec", pep_prec)
            self.tracker.log_scalar("predict/pep_recall", pep_recall)
            self.tracker.log_scalar("predict/auc", auc)
                ...
                eval_df = pd.DataFrame(
                    {
                        ...
                        "confidence": np.exp(log_probs),
                        "delta_mass_ppm": delta_mass_ppm,
                    }
                )
                self.tracker.log_mlflow_evaluation(
                    data=eval_df,
                    dataset_name=str(self.config.get("run_name", "predict_evaluation")),
                    context="predict_evaluation",
                    targets_col="target",
                    predictions_col="prediction",
                    confidence_col="confidence",
                    delta_mass_col="delta_mass_ppm",
        ...
        if self.using_groups and not self.denovo:
            ...
            for group in pred_df["group"].unique():
                ...
                if self.tracker is not None and self.config.get("mlflow_log_evaluations", True):
                    group_eval_df = pd.DataFrame(
                        {
                            "target": _group_targs,
                            "prediction": _group_preds,
                            "confidence": np.exp(_group_probs),
                        }
                    )
                    self.tracker.log_mlflow_evaluation(
                        data=group_eval_df,
                        dataset_name=f"{group}_predict_evaluation",
                        context=f"predict_evaluation_{group}",
                        targets_col="target",
                        predictions_col="prediction",
                        confidence_col="confidence",
                        peptide_metrics=self.metrics,
