...
class CosineHyperscoreCorrelationTask(BaseTask):
    ...
    def run(self, emb: np.ndarray, meta: Dict[str, np.ndarray], faiss_index: Any = None) -> Dict[str, Any]:  # type: ignore[override]  # base class run() signature differs across tasks
        ...
        correlation_results = self._compute_cosine_hyperscore_correlation(emb_sampled, peptides_sampled, processed_scores)
    ...
    def _compute_cosine_hyperscore_correlation(self, emb: np.ndarray, peptides: np.ndarray, scores: np.ndarray) -> Dict[str, Any]:
        ...
        valid_peptides = {peptide: indices for peptide, indices in peptide_groups.items() if len(indices) >= self.min_peptide_spectra}

        if not valid_peptides:
            return {"error": f"No peptides with at least {self.min_peptide_spectra} spectra found", "num_pairs": 0}
        ...
        for _peptide, indices in valid_peptides.items():
            if len(indices) < 2:
                continue
        ...
        if not mean_cosines:
            return {"error": "No valid cosine-score pairs found", "num_pairs": 0}
        ...
        try:
            ...
            return {"error": f"Failed to compute Spearman correlation: {e}", "num_pairs": len(mean_cosines)}
