...
DETECTORS = ["Orbitrap", "Iontrap", "Orbitrap|IonTrap", "TOF", "Triple Quadrupole", "Triple quadrupole", "Astral"]

ENZYMES = ["trypsin", "unspecific", "aspn", "lysn", "chymotrypsin", "lysc", "gluc"]
...
def _encode_categorical(
    ...
    """
    indices = [vocab_lookup.get(str(val).lower(), default_id) if val is not None else default_id for val in values]
...
def build_metadata_dict(
    ...
    if "precursor_mass" in batch[0]:
        ...
        precursor_mass_list = [float(sample.get("precursor_mass", 0.0)) if sample.get("precursor_mass") is not None else 0.0 for sample in batch]
        ...
        mz_list = [
            float(sample.get("precursor_mz", 0.0)) if ((mz := sample.get("precursor_mz")) is not None and isinstance(mz, (int, float))) else 0.0
            for sample in batch
        ]
        charge_list = [
            float(sample.get("precursor_charge", 1))
            if ((c := sample.get("precursor_charge")) is not None and isinstance(c, (int, float)) and c > 0)
            else 1.0
            for sample in batch
        ]
    ...
    if "collision_energy" in batch[0]:
        ...
        ce_list = [float(sample.get("collision_energy", CE_MIN)) if sample.get("collision_energy") is not None else CE_MIN for sample in batch]
    ...
    if "precursor_charge" in batch[0]:
        ...
        charge_list = [
            float(sample.get("precursor_charge", 2))
            if ((c := sample.get("precursor_charge")) is not None and isinstance(c, (int, float)) and c > 0)
            else 2.0
            for sample in batch
        ]
