...
class SimpleRotaryEmbedding(nn.Module):
    ...
    """
    def __init__(self, dim: int, max_seq_len: int = 2048, base: float = 10000.0, rotary_pct: float = 1.0):
        super().__init__()
        self.dim = dim
        self.base = base
    ...
    def clear_cache(self):
        ...
        """
        if self.rotary_dim > 0:
            max_seq_len = self._cos_cached.size(0)
            t = torch.arange(max_seq_len, device=self.inv_freq.device, dtype=self.inv_freq.dtype)
            freqs = torch.outer(t, self.inv_freq)
            emb = torch.cat((freqs, freqs), dim=-1)
            self._cos_cached = emb.cos().to(self._cos_cached.dtype)
            self._sin_cached = emb.sin().to(self._sin_cached.dtype)
