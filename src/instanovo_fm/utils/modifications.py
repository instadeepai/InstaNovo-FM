...
def clean_peptide_sequence(peptide: str) -> str:
    ...
    modifications = [r"\(ox\)", r"\(ph\)", r"\(ac\)", r"\(me\)", r"\(gly\)", r"\(glc\)", r"\(\+[0-9.]+\)", r"\(\-[0-9.]+\)"]
...
def compute_modification_types(peptides: np.ndarray, use_modified_peptide: bool = True) -> np.ndarray:
    ...
    for peptide in peptides:
        ...
        if peptide is None or not isinstance(peptide, str) or len(peptide.strip()) == 0 or peptide.strip().lower() == "none":
...
def compute_ptm_binary_flags(peptides: np.ndarray) -> dict:
    ...
    """
    if peptides is None or len(peptides) == 0:
        return {"mod_phospho": np.array([], dtype=np.int32), "mod_glyco": np.array([], dtype=np.int32)}
    phospho = np.fromiter((has_phosphorylation(p) for p in peptides), dtype=np.int32, count=len(peptides))
    glyco = np.fromiter((has_glycosylation(p) for p in peptides), dtype=np.int32, count=len(peptides))
...
def compute_glyco_deam_targets(peptides: np.ndarray) -> dict:
    ...
    """
    if peptides is None or len(peptides) == 0:
        return {"mod_deam_n": np.array([], dtype=np.int32), "glyco_class": np.array([], dtype=object)}
    deam_n = np.fromiter((has_deamidation_n(p) for p in peptides), dtype=np.int32, count=len(peptides))
